...
public class RetailStore {
  ...
  /**
   * TO Add Customer
   * @param name Passing Customer Name. 
   * @param phoneno Passing Customer Phone no.
   */
  public boolean addCustomer(String name, String phoneno) {
    Customer c1 = new Customer(name, phoneno);
    try {
      preparedStatement = connection
          .prepareStatement("INSERT INTO `customer`(`name`, `phoneno`) VALUES(?,?)");
  ...
  /**
   * Add Product
   * @param productName Passing Product Name.
   * @param quantity Passing Product Quantity.
   * @param price Passing Product Price.
   */
  public boolean addProduct(String productName, int quantity, float price) {
    Products product = new Products(productName, quantity, price);
    connection = DbConnectionUtil.getConnection();
    try {
      preparedStatement = connection.prepareStatement(
          "INSERT INTO `product`(`productname`, `quantity`, `price`) VALUES(?,?,?)");
   ...
   * Check Product
   * @param productname Passing Product Name.
   * @return QuantityOf Product.
   */
  public int checkProducts(String productname) {
    try (PreparedStatement preparedStatement = connection.prepareStatement(
        "SELECT quantity, COUNT(CASE WHEN quantity > 0 THEN 1 END) AS NumberOfGreaterThan0"
      ...
      if (prodCount > 0) {
        try (
            ...
            PreparedStatement prepareStatement2 = connection.prepareStatement(
                "INSERT INTO `bookedProduct`(`custId`, `products`, `quantity`,`price`)"
                + " VALUES(?,?,?,?,?)");
   ...
   * @param custId Passing Customer Id.
   * @param discount Passing Discount Amount.
   * @return Success Message.
   */
